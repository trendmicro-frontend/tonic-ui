...
#### Setting a minimum thumb size

Use the `minThumbSize` attribute to set the minimum thumb size (in pixels).

The thumb size will be set to the minimum thumb size if the thumb size is smaller than the minimum thumb size.
...
#### Setting a fixed thumb size

To set the fixed thumb size, use the `thumbSize` attribute.
...
| disabled | boolean | | Whether the scrollbar is disabled. |
| maxHeight | number \| string | 'auto' | The maximum height of the scrollable content. If set to 'auto', the height will be calculated based on the content. |
| minHeight | number \| string | 'auto' | The minimum height of the scrollable content. |
| minThumbSize | number | 32 | The minimum size of the thumb in pixels. |
...
| thumbSize | number | | The size of the thumb in pixels. |
