...
`MenuButton` composes the `Button` component to provide a menu trigger without having to interact with the menu state. It must be a direct child of `Menu` to function properly.

An indicator icon can be added to the `MenuButton` to indicate whether the menu is open or closed.
