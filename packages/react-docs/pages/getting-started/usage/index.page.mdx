...
Tonic UI can emit all theme tokens as `--tonic-*` CSS custom properties. Enable this with two steps: pass `cssVariables: true` to `createTheme`, and add the `useCSSVariables` prop to `TonicProvider`:

```jsx
const theme = createTheme({
  cssVariables: true, // Generate the CSS variable map
});

<TonicProvider
  theme={theme}
  useCSSVariables // Inject variables into the document and emit var(--tonic-*) in component styles
/>
```

Tonic UI uses colon-delimited token names (e.g. `gray:10`). In CSS variables, colons and dots are replaced with hyphens:
...
Once injected, these variables are available everywhere — in Emotion styles, plain CSS, and inline `style` attributes — without importing the theme object.

> See [CSS theme variables](../../customization/css-theme-variables) for prefix and shadow DOM options.
...
When `createTheme()` is used, `useTheme()` returns a resolved theme object with a `get(path, defaultValue)` helper. Use it to look up any token by its dot-separated path:
...
function MyComponent() {
  ...
  // Look up a color token (colon tokens use dot-path notation)
  const gray100 = theme.get('colors.gray:100');

  // Look up a typography token
  const smFontSize = theme.get('fontSizes.sm');

  // Provide a fallback for missing paths
  const fallback = theme.get('colors.brand.90', '#1a365d');

  return (
    <div style={{ color: gray100, fontSize: smFontSize }}>
      Hello
    </div>
  );
}
...
`theme.get()` accepts a default value as its second argument, returned when the path does not exist in the theme.

You can also use `useTheme()` to resolve tokens for a specific color mode without changing the active mode:

```jsx
// Always resolves tokens for light mode, regardless of the active color mode
const lightTheme = useTheme({ colorMode: 'light' });
const lightBg = lightTheme.get('colors.gray:10');
```

> `theme.get()` requires a theme created with `createTheme()`. For themes not created this way, use direct property access (`theme?.colors?.['gray:100']`) instead.
