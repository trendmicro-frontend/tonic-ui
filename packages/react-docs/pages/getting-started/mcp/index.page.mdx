...
import roocodeMCPServersImage from './roocode/mcp-servers.png';
import CopilotSVGIcon from './copilot/copilot.svg';
import copilotMCPStartButtonImage from './copilot/mcp-start-server-button.webp';
import copilotChatAgentOptionImage from './copilot/copilot-chat-agent-option.webp';
...
<Image src={roocodeMCPServersImage.src} />
...
3. A "Start" button will appear in your `.vscode/mcp.json` file, at the top of the list of servers. Click the "Start" button to start the MCP servers. This will trigger the input dialog and discover the server tools, which are then stored for later sessions.
    ...
    <Image src={copilotMCPStartButtonImage.src} width="400px" />

4. Open Copilot Chat by clicking the <CopilotSVGIcon /> icon in the title bar of Visual Studio Code.

5. In the Copilot Chat box, select **Agent** from the popup menu.

    <Image src={copilotChatAgentOptionImage.src} width="400px" />
