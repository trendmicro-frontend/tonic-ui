...
`PseudoBox` is now deprecated and its props can now be passed to `Box` directly. Replace all `PseudoBox` components with `Box`.
...
`ToggleSwitch` is now deprecated. Replace all `ToggleSwitch` components with `Switch`.
...
For example, you don't need to manually add transitions to `ModalOverlay` and `ModalContent` because they already have built-in transitions.
