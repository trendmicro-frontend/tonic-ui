...
- `DatePickerContent`: A wrapper that contains the calendar.
...
Use the `weekStartDay` prop to chage the first day of the calendar week. for example, 0 = Sunday, 1 = Monday.
...
Use the `dateFormat` prop to change the date format.

See https://www.unicode.org/reports/tr35/tr35-dates.html#Date_Field_Symbol_Table for a full list of date format symbols.
...
`DatePicker` is used to select a date from a calendar or type in a date value manually. It is composed of `DateInput` and `Calendar`.
...
| :--- | :--- | :------ | :---------- |
| weekStartDay | number | 0 | Set the first day of the calendar week. for example, 0 = Sunday, 1 = Monday. |
...
| Name | Type | Default | Description |
| :--- | :--- | :------ | :---------- |
| children | ReactNode \| ({ getDatePickerToggleProps }) => ReactNode | | |
