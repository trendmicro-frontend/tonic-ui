...
const drawerWithBodyScrollLockCode = `
<Drawer>
  <Global
    styles={css\`
      body {
...
function Example() {
  ...
  const [enableDrawerOverflowBehavior, toggleDrawerOverflowBehavior] = useToggle(true);
    ...
    <>
      ...
      <FormGroup>
        <TextLabel display="flex" alignItems="center" mb="3x">
          <Checkbox
            checked={enableDrawerOverflowBehavior}
            onChange={() => toggleDrawerOverflowBehavior()}
          />
          <Space width="2x" />
          <Text>
            Enable drawer overflow behavior to make scrollbar visible
          </Text>
        </TextLabel>
        <Box ml="6x">
          <Text mb="2x">
            Enable this option will increase the content height to overflow the viewport and make the <Code>DrawerBody</Code> scrollable.
          </Text>
        </Box>
      </FormGroup>
      <FormGroup>
