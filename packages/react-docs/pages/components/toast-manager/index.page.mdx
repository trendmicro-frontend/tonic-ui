...
To control the spacing to the edges of the screen, pass `sx` to the transition slot via `slotProps.transition`.
...
| TransitionComponent | ElementType | ToastTransition | **Deprecated.** Use `slots.transition` instead. |
