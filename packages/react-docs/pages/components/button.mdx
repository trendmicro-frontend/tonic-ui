...
If the `selected` prop is set (or set to `true`), the button will have a `pointer-events: none` style and a `tabIndex="-1"` attribute. This will prevent the button from receiving pointer events and will not be reachable via sequential keyboard navigation.
