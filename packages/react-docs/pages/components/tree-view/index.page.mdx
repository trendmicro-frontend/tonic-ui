...
To enable multi-selection in the TreeView component, you must configure the following properties:

* `isSelectable`: Set this property to `true` to allow nodes to be selected.
* `multiSelect`: Set this property to `true` to enable multi-selection with `ctrl` and `shift` keys.
