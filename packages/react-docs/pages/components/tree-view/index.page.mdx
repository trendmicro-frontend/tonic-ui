...
| Name | Type | Default | Description |
| :--- | :--- | :------ | :---------- |
| ContentComponent | ElementType | TreeItemContent | The component used to render the content. |
...
| render | `(props: TreeItemRenderProps) => ReactNode` | | `REQUIRED` A function component to render the tree item.<br /><pre>type TreeItemRenderProps = {'{'}<br />  nodeDepth: number;<br />  nodeId: string;<br />  isDisabled: boolean;<br />  isExpandable: boolean;<br />  isExpanded: boolean;<br />  isFocused: boolean;<br />  isSelected: boolean;<br />  select: () => void;<br />  selectRange: () => void;<br />  toggleExpansion: () => void;<br />  toggleSelection: () => void;<br />{'};'}</pre> |
