...
### Resolving popover content cut-off with `portalled`
...
To address this issue, you can pass `portalled` to `Popover` to render it on the document root.
...
| placement | string | 'bottom' | The preferred placement of the popover; Popper.js may choose a different placement when the `flip` modifier is enabled. One of: 'top', 'bottom', 'right', 'left', 'top-start', 'top-end', 'bottom-start', 'bottom-end', 'right-start', 'right-end', 'left-start', 'left-end' |
| portalled | boolean | false | If `true`, renders the popover in a portal. |
...
| PopoverArrowComponent | ElementType | PopoverArrow | **Deprecated.** Use `slots.arrow` instead. |
| PopoverArrowProps | object | | **Deprecated.** Use `slotProps.arrow` instead. |
