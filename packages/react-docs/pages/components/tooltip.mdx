...
To hide the tooltip, you can pass an empty value to the `label` prop or use the `disabled` prop.

#### Use the `label` prop to hide the tooltip

> Empty values include `null`, `undefined`, `''` (empty string), `' '` (blank string), `[]`, and `{}`.
