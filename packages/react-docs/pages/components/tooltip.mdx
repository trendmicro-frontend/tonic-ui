...
To hide the tooltip, you can pass a blank or empty string to the `label` prop, or pass the `disabled` prop.
...
| label | string \| ReactNode | | The tooltip label. If the label string is blank or empty, the tooltip will not display. |
