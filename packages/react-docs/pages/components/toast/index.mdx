...
For the Hook version, use the `useToast` Hook to get all the methods, properties, and state. See [useToast](toast/useToast) Hook for detailed usage.
...
const CustomToastContainer = (props) => (
  ...
  >
    <Box
      ...
      maxWidth="80%" // The maximum width is 80% of the modal or drawer width
      margin="0 auto"
      {...props}
    />
  </Flex>
