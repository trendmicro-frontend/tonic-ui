...
function Example() {
  ...
  const updateIndicatorOpacity = () => {
    const { scrollLeft, scrollWidth, clientWidth } = tabScrollerRef.current;
    // use 1 for the potential rounding error with browser zooms.
    startScrollOpacity = 1 / 20 * Math.min(scrollLeft, 20);
    endScrollOpacity = 1 / 20 * ((scrollWidth - clientWidth) - Math.max(scrollLeft, (scrollWidth - clientWidth - 20)));
    console.log('###', startScrollOpacity, endScrollOpacity);

    if (startScrollIndicatorRef.current) {
      console.log('### startScrollIndicatorRef:', startScrollOpacity, startScrollIndicatorRef.current);
      startScrollIndicatorRef.current.style.opacity = startScrollOpacity;
    }
    if (endScrollIndicatorRef.current) {
      console.log('### endScrollIndicatorRef:', endScrollOpacity, endScrollIndicatorRef.current);
