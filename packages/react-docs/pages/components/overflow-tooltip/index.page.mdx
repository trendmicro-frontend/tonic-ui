...
### Preventing tooltip cut-off with `portalled`
...
To prevent this, set `portalled` on the `OverflowTooltip`. This will position the tooltip at the document root.
...
To resolve this issue, set `portalled` on the `OverflowTooltip`.
