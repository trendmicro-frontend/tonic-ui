...
By default, the `OverflowTooltip` is positioned adjacent to the cursor. However, within a popup menu, the tooltip may not align properly with the menu items. To correct this issue, set the `nextToCursor` property to `false`.
