...
### OverflowTooltip misalignment with popup menu items

By default, the `OverflowTooltip` is positioned next to the cursor. However, within a popup menu, the tooltip may not align correctly with the menu items. To fix this, set `PopperProps={{ usePortal: true }}` and `nextToCursor={false}`.

{render('./with-menu-item')}
