  ...
  return (
    <>
      ...
      <FormGroup>
        ...
        <Box ml="6x">
          <Text mb="2x">
            When setting <Code>scrollBehavior="outside"</Code> to enable outside scrolling, you should also use <strong>Body Scroll Locking</strong> to prevent the user from scrolling the page while the modal is open. Body scroll locking is currently not available with default setup, you can use <Code>{`import { Global } from '@emotion/react';`}</Code> to dynamically inject the global style as shown below:
          </Text>
          <pre>
