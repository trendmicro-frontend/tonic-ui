...
function Example() {
  ...
  const [minHeight, changeMinHeightBy] = useSelection('default');
  ...
  if (minHeight !== 'default') {
    modalContentStyleProps.minHeight = minHeight;
  }
  ...
  return (
    <>
      ...
      <FormGroup>
        ...
        >
          {['default', 'calc(100vh - 6rem)', '100%'].map(value => (
            <SelectButton
              key={value}
              isSelected={value === minHeight}
              onClick={changeMinHeightBy(value)}
              minWidth="15x"
            >
              {value}
            </SelectButton>
