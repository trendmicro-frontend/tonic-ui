...
* `rect` and `circle` represent a rectangle and a circle respectively.

```jsx
<Stack direction="column" spacing="4x" width={240}>
  <Skeleton variant="text" />
  <Skeleton variant="circle" width={40} height={40} />
  <Skeleton variant="rect" width={240} height={80} />
  <Skeleton variant="rect" width={240} height={80} borderRadius="sm" />
</Stack>
```
...
```jsx
<Stack direction="column" width="max(320px, 50%)">
  <Flex alignItems="center" columnGap="5x" backgroundColor="gray:90" p="4x">
    <Flex flex="none">
