...
By default, the submenu is rendered within the same container as its parent menu. To avoid issues with `overflow: hidden`, you can render the submenu in a portal by passing `usePortal: true` to the `PopperProps` of `SubmenuList`.
