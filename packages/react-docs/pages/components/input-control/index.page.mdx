...
| slots | object | `{}` | The slot components. Available slots: `input`, `root`. |
| slotProps | object | `{}` | The props for each slot. Available slots: `input`, `root`. |
| inputComponent | ElementType | InputBase | **Deprecated.** Use `slots.input` instead. |
| inputProps | object | | **Deprecated.** Use `slotProps.input` instead. |
