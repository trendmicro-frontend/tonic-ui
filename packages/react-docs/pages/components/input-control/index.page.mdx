...
| inputComponent *(deprecated)* | ElementType | InputBase | The input component to render.<br />⚠️ This prop is deprecated and will be removed in a future release. Use `slots.input` instead. |
| inputProps *(deprecated)* | object | | Props applied to the input element.<br />⚠️ This prop is deprecated and will be removed in a future release. Use `slotProps.input` instead. |
| inputRef | RefObject | | A `ref` to access the input element. |
| slots | object | | Slot components. `slots.input` replaces `inputComponent`; `slots.root` replaces the outer `<Box>`. |
| slotProps | object | | Props for each slot. `slotProps.input` replaces `inputProps`; `slotProps.root` for the outer container. |
