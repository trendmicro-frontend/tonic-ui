...
Use the `variantColor` prop to change the color scheme of a radio button. `variantColor` can be any color key with key `500` (hover) or `600` (checked) that exist in `theme.colors`.
