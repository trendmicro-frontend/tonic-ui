...
| :--- | :--- | :------ | :---------- |
| children | ReactNode | | |
| onResize | `({ clientX, clientY })` | | A callback function that is called when the resize handle is dragged. |
| onResizeEnd | `({ clientX, clientY })` | | A callback function that is called when the resize handle is released. |
| onResizeStart | `({ clientX, clientY })` | | A callback function that is called when the resize handle is clicked. |
