...
You can use either `react-movable` or `react-dnd` to enable row reordering.

The following example uses `react-movable` to enable row reordering:
...
The following example uses `react-dnd` to enable row reordering:
