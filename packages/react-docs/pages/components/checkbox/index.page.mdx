...
Use the `variantColor` prop to change the color scheme of the checkbox. `variantColor` can be any color key with key `500` (hover) or `600` (checked, active) that exist in `theme.colors`.
