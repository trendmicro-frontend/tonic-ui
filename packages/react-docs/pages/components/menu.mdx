...
function menuWithCheckbox() {
  const allItems = ['host', 'ip', 'os'];
  const mapItemToText = (item) => ({
    'host': 'Endpoint name',
    'ip': 'IP address',
    'os': 'Operating System',
  }[item]);
  const [checkedItems, setCheckedItems] = React.useState(allItems);
  const isAllSelected = checkedItems.length === allItems.length;
  const isNoneSelected = checkedItems.length === 0;
  const isIndeterminate = !isAllSelected && !isNoneSelected;
  const handleClickToggleCheckedItems = (event) => {
    const nextCheckedItems = isAllSelected ? [] : allItems;
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          ...
          {isIndeterminate && (
            <>
              <Truncate>
                {checkedItems.map(mapItemToText).join(', ')}
              </Truncate>
              <Text>
                ({checkedItems.length})
              </Text>
      ...
      >
        ...
        >
          <LinkButton
            onClick={handleClickToggleCheckedItems}
          >
        ...
        >
          {['host', 'ip', 'os'].map(x => (
            <MenuItem key={x} onKeyDown={handleKeyDown}>
              <Checkbox value={x} width="100%">
                {mapItemToText(x)}
              </Checkbox>
            </MenuItem>
          ))}
