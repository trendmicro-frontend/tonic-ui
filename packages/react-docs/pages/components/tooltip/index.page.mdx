...
> By default, the tooltip's offset is set to `[0, 8]`, where the first value represents the `skidding` and the second value represents the `distance`.<br/>In this example, the `distance` value is set to 16. You may need to adjust this value to better suit your specific requirements.
...
| arrow | boolean | true | If `true`, an arrow will be added to the tooltip. Note that the arrow will not be visible if either `followCursor` or `nextToCursor` is set to `true`. |
