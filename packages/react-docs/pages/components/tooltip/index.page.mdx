...
### Resolving tooltip content cut-off with `portalled`
...
To address this issue, you can pass `portalled` to `Tooltip` to render it on the document root.
...
| :--- | :--- | :------ | :---------- |
| slots | object | `{}` | The slot components. Available slots: `arrow`, `popper`, `transition`. |
| slotProps | object | `{}` | The props for each slot. Available slots: `arrow`, `popper`, `transition`. |
| TooltipArrowComponent | ElementType | TooltipArrow | **Deprecated.** Use `slots.arrow` instead. |
| TooltipArrowProps | object | | **Deprecated.** Use `slotProps.arrow` instead. |
...
| TransitionComponent | ElementType | Grow | **Deprecated.** Use `slots.transition` instead. |
...
| placement | PopperJS.Placement | 'bottom' | The preferred placement of the tooltip; Popper.js may choose a different placement when the `flip` modifier is enabled. One of: 'top', 'bottom', 'right', 'left', 'top-start', 'top-end', 'bottom-start', 'bottom-end', 'right-start', 'right-end', 'left-start', 'left-end' |
| portalled | boolean | false | If `true`, renders the tooltip in a portal. |
