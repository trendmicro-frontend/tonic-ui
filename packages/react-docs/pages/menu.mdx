...
- `MenuButton`: A button that composes `MenuToggle` and `Button` and is used to open the menu.
...
`MenuButton` composes `MenuToggle` and `Button` to create a button that opens the menu. It's an useful alternative to `MenuToggle` when the toggle is a button.
