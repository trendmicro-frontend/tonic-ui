...
To get color mode workding correctly, you may have to pass `colorMode` config to `TonicProvider` with `defaultValue` set to `dark` or `light`.
...
To manage color mode in your application, you can use the `useColorMode` hook.
