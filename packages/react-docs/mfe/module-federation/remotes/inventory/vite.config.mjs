...
// Inventory REMOTE. Built statically (no dev server) into
// public/mfe/module-federation/inventory/. `base` is the same-origin path Next
// serves it from, so the emitted mf-manifest.json + remoteEntry resolve via
// relative URLs when the shell fetches them.
export default defineConfig({
  root: dirname,
  base: `${basePath}/mfe/module-federation/inventory/`,
  plugins: [
    federation({
      name: 'inventory',
  ...
  build: {
    outDir: path.resolve(dirname, '../../../../public/mfe/module-federation/inventory'),
    emptyOutDir: true,
    target: 'esnext',
    minify: false,
    rollupOptions: {
      // See dashboard remote: externalize the Vite 8+ subpath the MF runtime plugin
