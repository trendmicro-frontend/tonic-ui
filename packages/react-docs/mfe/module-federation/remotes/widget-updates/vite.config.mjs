...
// UpdateRecommendedWidget REMOTE. Built statically (no dev server) into
// public/mfe/module-federation/widget-updates/. `base` is the same-origin path
// Next serves it from, so the emitted mf-manifest.json + remoteEntry resolve via
// relative URLs when the shell fetches them.
export default defineConfig({
  root: dirname,
  base: `${basePath}/mfe/module-federation/widget-updates/`,
  plugins: [
    federation({
      name: 'widget_updates',
  ...
  build: {
    outDir: path.resolve(dirname, '../../../../public/mfe/module-federation/widget-updates'),
    emptyOutDir: true,
    target: 'esnext',
    minify: false,
    rollupOptions: {
