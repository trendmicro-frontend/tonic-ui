...
Tonic Styled UI comes with a color style system that defines functional color values. The color values are defined in [color-style.js](https://github.com/trendmicro-frontend/styled-ui/blob/master/packages/react-styled-ui/src/ColorStyleProvider/color-style.js). You can override the default value by creating a color style object in accordance to the color mode, and pass the object to `ColorStyleProvider` with the `value` prop. For example:
...
Then you can use the `useColorStyle` Hook to get [color style](https://github.com/trendmicro-frontend/styled-ui/blob/master/packages/react-styled-ui/src/ColorStyleProvider/color-style.js) for a given color mode.
