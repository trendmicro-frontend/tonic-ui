...
function Example() {
  ...
  const [closeOnOutsideClick, toggleCloseOnOutsideClick, setCloseOnOutsideClick] = useToggle(false);
  const [isOverlayVisible, toggleIsOverlayVisible, setIsOverlayVisible] = useToggle(false);
...
The drawer doesn't come with a transition by default. You can use transition components to provide simple transitions.
