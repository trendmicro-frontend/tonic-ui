# Transitions

Transition helps make a UI expressive and easy to use. The Transition components use `react-transition-group` internally to perform animation effects and manage component states (including mounting and unmounting) over time.

## Import

```js
import {
  Collapse,
  Fade,
  Scale,
  Slide,
  Zoom,
} from '@trendmicro/react-styled-ui';
```

## Usage

### Collapse

Expand vertically from the top of the children. The `collapsedHeight` property can also be used to set the minimum height when collapsed.

```jsx
function Example() {
  const { isOpen, onToggle } = useDisclosure();

  return (
    <>
      <Button onClick={onToggle} mb="4x">
        Toggle
      </Button>
      <Collapse in={isOpen} collapsedHeight={0}>
        <SkeletonContent>
          <SkeletonBody />
        </SkeletonContent>
      </Collapse>
    </>
  );
}
```

### Fade

```jsx
function Example() {
  const { isOpen, onToggle } = useDisclosure();

  return (
    <>
      <Button onClick={onToggle} mb="4x">
        Toggle
      </Button>
      <Fade in={isOpen} unmountOnExit>
        <SkeletonContent>
          <SkeletonBody />
        </SkeletonContent>
      </Fade>
    </>
  );
}
```

### Scale

```jsx
function Example() {
  const { isOpen, onToggle } = useDisclosure();

  return (
    <>
      <Button onClick={onToggle} mb="4x">
        Toggle
      </Button>
      <Scale in={isOpen} unmountOnExit>
        <SkeletonContent>
          <SkeletonBody />
        </SkeletonContent>
      </Scale>
    </>
  );
}
```

### Slide

```jsx noInline
const useSelection = (defaultValue) => {
  const [value, setValue] = React.useState(defaultValue);
  const changeBy = (value) => () => setValue(value);
  return [value, changeBy];
};

function Example() {
  const { isOpen, onClose, onToggle } = useDisclosure();
  const [direction, changeDirectionBy] = useSelection('up');

  return (
    <>
      <Box mb="4x">
        <Button onClick={onToggle}>
          Toggle
        </Button>
      </Box>
      <ButtonGroup
        variant="secondary"
        css={{
          '> *:not(:first-of-type)': {
            marginLeft: -1
          }
        }}
      >
        {['up', 'down', 'left', 'right'].map(value => {
          const changeDirection = changeDirectionBy(value);
          const onClick = () => {
            changeDirection();
            onClose();
          };

          return (
            <SelectableButton
              key={value}
              selected={value === direction}
              onClick={onClick}
              minWidth="15x"
            >
              {value}
            </SelectableButton>
          );
        })}
      </ButtonGroup>
      <Slide in={isOpen} direction={direction} unmountOnExit>
        <SkeletonContent mt="4x">
          <SkeletonBody />
        </SkeletonContent>
      </Slide>
    </>
  );
}

render(<Example />);
```

### Zoom

```jsx
function Example() {
  const { isOpen, onToggle } = useDisclosure();

  return (
    <>
      <Button onClick={onToggle} mb="4x">
        Toggle
      </Button>
      <Zoom in={isOpen} unmountOnExit>
        <SkeletonContent>
          <SkeletonBody />
        </SkeletonContent>
      </Zoom>
    </>
  );
}
```

## Props

### Collapse

The Collapse transition is used by the `Accordion` component. It uses `react-transition-group` internally.

### Fade

The Fade transition is be used by the `Modal` component. It uses `react-transition-group` internally.

| Name | Type | Default | Description |
| :--- | :--- | :------ | :---------- |
| in | boolean | | Toggle visibility with a transition effect. |

### Scale

The Scale transition is be used by the `Tooltip` and `Popover` components. It uses `react-transition-group` internally.

| Name | Type | Default | Description |
| :--- | :--- | :------ | :---------- |
| in | boolean | | Toggle visibility with a transition effect. |
| initialScale | number | 0.95 | The initial scale level. |

### Slide

The Slide transition is used by the `Drawer` component. It uses `react-transition-group` internally.

| Name | Type | Default | Description |
| :--- | :--- | :------ | :---------- |
| in | boolean | | Toggle visibility with a transition effect. |
| direction | string | | One of: `'up'`, `'down'`, `'left'`, `'right'` |

### Zoom

The Zoom transition is used for zooming in/out the displayed components. It uses `react-transition-group` internally.
