...
| :--- | :--- | :------ | :---------- |
| children | ReactNode \| (state, { ref, style, ...childProps }) => ReactNode | | A function child can be used instead of a React element. This function is called with the current transition state (`'entering'`, `'entered'`, `'exiting'`, `'exited'`), ref, style, and child props, which can be used to apply context specific props to a component. |
