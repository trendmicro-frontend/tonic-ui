...
For the Hook version, use the `useToast` Hook to get all the toast functions. See the <b>Toast API</b> section for detailed usage.
...
* `[options.duration]` *(number)*: The duration that the toast should remain on the screen.
...
* `[options={}]` *(Object)*: The options object.
* `[options.placements=[]]` *(Array)*: An array of toast placements.
