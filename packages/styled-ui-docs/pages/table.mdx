    ...
    <Box height={300}>
      <AutoSizer>
        {({ height, width }) => {
          ...
          const customWidthColumns = columns.filter(column => column.width != 'auto');
          const totalCustomWidth = customWidthColumns.reduce((accumulator, column) => accumulator + column.width, 0);
          const defaultCellWidth = (width - totalCustomWidth) / (columns.length - customWidthColumns.length);
          return (
            <Table
              isHoverable
              {...getTableProps()}
