...
```js
import Text from '@trendmicro/react-styled-ui/Text';
// or
...
You can format the `::selection` style by passing `__selection`.
...
`Text` composes the [`PseudoBox`](./pseudoBox) component. You can override the default styles with style props.
