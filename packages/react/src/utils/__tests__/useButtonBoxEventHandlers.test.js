...
function TestButtonBox({ disabled, onActivate }) {
  const handlers = useButtonBoxEventHandlers({ disabled, onActivate });
  return (
    <Box
      role="button"
...
describe('useButtonBoxEventHandlers', () => {
  ...
  test('calls onActivate on Space key up (no repeat)', () => {
    const onActivate = jest.fn();
    render(<TestButtonBox onActivate={onActivate} />);
  ...
  test('does not call onActivate on Space key down', () => {
    const onActivate = jest.fn();
    render(<TestButtonBox onActivate={onActivate} />);
  ...
  test('does not call onActivate on repeated Space key up', () => {
    const onActivate = jest.fn();
    render(<TestButtonBox onActivate={onActivate} />);
    fireEvent.keyUp(screen.getByRole('button'), { key: ' ', repeat: true });
    expect(onActivate).not.toHaveBeenCalled();
  });
