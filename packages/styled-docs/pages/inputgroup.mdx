...
For an unstyled input group that doesn't have border and padding spaces, it is not necessary to place an `InputPrepend` or `InputAppend` on either side of an `Input`.
