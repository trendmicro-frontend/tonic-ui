...
* Use the `variantColor` prop to change the background color of the tooltip. `variantColor` can be any **color key with key 60** that exist in the `theme.colors`.

  > That means the `red:60` exists in the `theme.colors` then can use variantColor="red".
* [Learn more about colors theming](./colors).

```jsx
<Tooltip label="Search places" placement="top" variantColor="red">
