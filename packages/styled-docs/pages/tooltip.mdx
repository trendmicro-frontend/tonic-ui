...
```jsx
<Tooltip hasArrow={false} label="Search places" placement="top">
...
can be any color key that exists in the `theme.colors`. [Learn more about color theming](./colors).

```jsx
<Tooltip label="Search places" placement="top" variantColor="red:60">
...
| `hasArrow`           | `boolean`            | `true`   | If `true` display an arrow tip on the tooltip.                                             |
