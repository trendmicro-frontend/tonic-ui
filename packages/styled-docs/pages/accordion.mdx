...
Trend Micro Styled UI exports 5 accordion related components.
...
The `AccordionItem` component has the render prop `isExpanded` set to `true` or `false`. Use it to style this state.
...
style it by passing the pseudo style props `_disabled`, `_hover`,
etc.
