...
Stack is a layout utility component that makes it easy to stack items together and apply a space between them. It composes the [Flex](./flex) component.
...
const Item = (props) => {
  ...
  const boxShadow = colorMode === 'dark' ? '1x.dark' : '1x.light';
  const borderColor = colorMode === 'dark' ? 'gray.50' : 'gray.10';
  return (
    <Box p=".5rem" boxShadow={boxShadow} border={1} borderColor={borderColor} {...props} />
  );
};
