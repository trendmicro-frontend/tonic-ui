# Getting Started

Install Trend Micro Styled UI and its peer dependencies.

```bash
npm install @trendmicro/react-styled-core @trendmicro/styled-theme
```

Create a theme object to include custom color, typography, and layout values.
Styled UI comes with a default theme, so this step is optional.

```js
// example base.js
export default {
  fonts: {
    heading: '-apple-system, BlinkMacSystemFont, "Segoe UI", "Helvetica Neue", Helvetica, Arial, sans-serif',
    base: '-apple-system, BlinkMacSystemFont, "Segoe UI", "Helvetica Neue", Helvetica, Arial, sans-serif',
    mono: '"SFMono-Medium", "SF Mono", "Segoe UI Mono", Menlo, Consolas, Courier, monospace',
  };
  fontSizes: {
    xs: '.75rem',
    sm: '.875rem',
    md: '1rem',
    lg: '1.125rem',
    xl: '1.25rem',
    '2xl': '1.5rem',
    '3xl': '1.75rem',
    '4xl': '2rem',
  },
};
```

You can also extend the theme to add your custom colors, fonts, etc.

```js
import { theme } from '@trendmicro/styled-theme';

// Let's say you want to add custom colors
const customTheme = {
  ...theme,
  colors: {
    ...theme.colors,
    brand: {
      90: "#1a365d",
      80: "#153e75",
      70: "#2a69ac",
    },
  },
};
```

Add the theme to your application with the `ThemeProvider`, passing in the theme
object as a prop.

```js
import React from 'react';
import { ThemeProvider } from '@trendmicro/react-styled-core';

// Use at the root of your app
<ThemeProvider theme={customTheme}>{props.children}</ThemeProvider>;
```

## Using components

Simply import components using default imports or named imports from `@trendmicro/react-styled-core`, as shown below:

### Default import

```js
import Button from '@trendmicro/react-styled-core/Button';
```

### Named import

```js
import { Button } from '@trendmicro/react-styled-core';
```
